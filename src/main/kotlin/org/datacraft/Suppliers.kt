...
object Suppliers {
    ...
    fun values(spec: FieldSpec): ValueSupplier<Any> {
        return when (spec.data) {
            is List<*> -> {
                ...
                ListValueSupplier(spec.data as List<Any>)
            }

            is Map<*, *> -> {
                @Suppress("UNCHECKED_CAST")
                WeightedValueSupplier(spec.data as Map<Any, Any>)
            }

            else -> {
                ConstantValueSupplier(spec.data)
            }
        }
    }
    ...
    class ListValueSupplier<T>(private val values: List<T>) : ValueSupplier<T> {
         ...
         */
        override fun next(iteration: Int): T = values[iteration % values.size]
    }
    ...
    class ConstantValueSupplier<T>(private val value: T) : ValueSupplier<T> {
        override fun next(iteration: Int): T = value
    }
    ...
    class WeightedValueSupplier(private val weights: Map<Any, Any>) : ValueSupplier<Any> {
         ...
         */
        override fun next(iteration: Int): Any {
